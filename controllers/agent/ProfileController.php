...
public function uploadAvatar($params = []) {
    ...
    if ($file['error'] !== UPLOAD_ERR_OK) {
        http_response_code(400);
        jsonResponse(['error' => 'Upload error code: ' . $file['error']]);
        return;
    }
    ...
    $url = uploadToCloudinary($file['tmp_name'], 'avatars');

    $model = new Profile($this->db);
    $model->updateAvatar((int) $this->user['id'], $url);

    jsonResponse(['success' => true, 'avatar_url' => $url]);
}
