...
class ListingController {
    ...
    public function uploadPhotos($params = []) {
        ...
        // Normalise $_FILES['photos'] into a flat list
        $files = $_FILES['photos'];
        ...
        // Check if this listing already has a cover photo
        ...
        foreach ($uploaded as $i => $urlPath) {
            ...
            $stmt = $this->db->prepare(
                "INSERT INTO listing_photos (listing_id, photo_url, is_cover, sort_order)
                 VALUES (?, ?, ?, ?)"
            );
            $stmt->execute([$params['id'], $urlPath, $isCover, $i]);
            if ($isCover) $hasCover = true; // only first gets cover
        }
    ...
    public function uploadDocuments($params = []) {
        ...
        $uploadDir = __DIR__ . '/../../public/uploads/listings/' . $params['id'] . '/docs/';
        if (!is_dir($uploadDir)) mkdir($uploadDir, 0755, true);
        ...
        foreach ($fileList as [$name, $tmp, $type, $err]) {
            ...
            if (move_uploaded_file($tmp, $uploadDir . $filename)) {
                $uploaded[] = [
                    'name' => $name,
                    'url'  => '/uploads/listings/' . $params['id'] . '/docs/' . $filename,
                ];
            }
        }
